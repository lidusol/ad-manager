...
import { Search, AdGroupSearch, ResponsiveSearchAd } from '../models/search.models';
import { SortColumn, SortDirection } from 'src/app/layout/directives/sortable.directive';
import { DecimalPipe } from '@angular/common';
import { BehaviorSubject, Observable, of, Subject } from 'rxjs';
declare var require: any;
const compare = (v1: string | number, v2: string | number) => v1 < v2 ? -1 : v1 > v2 ? 1 : 0;

function sort(countries: KEYWORD_RESULT[], column: SortColumn, direction: string): KEYWORD_RESULT[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: KEYWORD_RESULT, term: string, pipe: PipeTransform) {
  return country.keyword.toLowerCase().includes(term.toLowerCase())
...
export class SearchService {
  ...
  private _countries$ = new BehaviorSubject<KEYWORD_RESULT[]>([]);
  ...
  private uid: string = "";
  private campaignSearch: Display;
  private adsModel: ResponsiveSearchAd;
  public campaignSearchCollection: AngularFirestoreCollection<Search>;
  public adGroupCollection: AngularFirestoreCollection<AdGroupSearch>;
  public annonceCollection: AngularFirestoreCollection<ResponsiveSearchAd>;
  ...
  public KEYWORDS: KEYWORD_RESULT[] = []
  constructor(private http: HttpClient, private auth: AuthService, public afs: AngularFirestore, private pipe: DecimalPipe) {
    this.campaignSearchCollection = this.afs.collection('adwords-search', (ref) => ref.where('owner', '==', this.uid));
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let countries = sort(this.KEYWORDS, sortColumn, sortDirection);
  ...
  public getCampaignName(uid: string, name: string) {
    return this.afs.collection<Search>('adwords-search', (ref) => ref.where('owner', '==', uid).where('name', '==', name)).snapshotChanges().pipe(
      map((actions) => {
   ...
   */
  saveDuplicateCampaign(display: Display): Promise<Search> {
    return new Promise(resolve => {
      this.afs.collection('adwords-search').add(display).then(added => {
   ...
   * @param  {Display} display
   */
  createCampaign(display: Search): Promise<string> {
    return new Promise(resolve => {
      this.campaignSearch = this.prepareSaveCampaignSearch(display);
      const docRef = this.campaignSearchCollection.add(this.campaignSearch);
      docRef
        .then((add) => {
          resolve(add.id)
  ...
  prepareSaveCampaignSearch(display: Search): Search {
    const newCampaign = display
    return { ...newCampaign };
  }
  /**
  ...
  prepareSaveAdGroupSearch(uid: string, accountId: string, campaign_id: number, name: string, status: string, ad_group_id: number): AdGroupSearch {
    const userDoc = this.afs.doc(`users/${uid}`);
    const newAdGroup: AdGroupSearch = {
      campaign_id: campaign_id,
      ad_group_id: ad_group_id,
      name: name,
      status: status,
  ...
  public getListCampaign(uid: string, accountId: string) {
    return this.afs.collection<Search>('adwords-search', (ref) => ref.where('owner', '==', uid).where('accountId', '==', accountId).where('isArchived', '==', false).where('isExpress', '==', true)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
  ...
  public getListCampaignYoutube(uid: string, accountId: string) {
    ////////////////console.log(uid)
    return this.afs.collection<Search>('youtube-ads', (ref) => ref.where('owner', '==', uid).where('accountId', '==', accountId).where('isArchived', '==', false)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  public getCampaignWithAdGroupId(ad_group_id: number, uid: string) {
    return this.afs.collection<Search>('adwords-search', (ref) => ref.where('owner', '==', uid).where('ad_group_id', '==', ad_group_id).where('isArchived', '==', false).where('isExpress', '==', true)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  public getListAds(uid: string, accountId: string) {
    ...
    return this.afs.collection<Ads>('ads-search', (ref) => ref.where('owner', '==', uid).where('accountId', '==', accountId)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  public getListAdsWithAdGroupId(ad_group_id: number) {
    ////////////console.log(uid)
    ////////////console.log(accountId)
    return this.afs.collection<Ads>('ads-search', (ref) => ref.where('ad_group_id', '==', ad_group_id)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  getCampaign(id: string) {
    return this.afs.doc<Search>(`adwords-search/${id}`);
  }
  ...
  createAdGroup(uid: string, accountId: string, id_campagne: number, name: string, status: string, ad_group_id: number): Promise<any> {
    return new Promise(resolve => {
      this.adgroup = this.prepareSaveAdGroupSearch(uid, accountId, id_campagne, name, status, ad_group_id);
      const docRef = this.afs.collection('adgroup-search').add(this.adgroup).then(res => {
  ...
  getSingleAdGroupID(campaign_id: number, ad_group_id: number) {
    ...
    return this.afs.collection<AdGroupSearch>('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  promiseGetListAdGroupId(campaign_id) {
    return this.afs.collection<AdGroupSearch>('adgroup-search', (ref) => ref.where('campaign_id', '==', parseInt(`${campaign_id}`))).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  getCampaignListID(uid: string, accountId: string): Promise<Display[]> {
    return new Promise(resolve => {
      this.getListCampaign(uid, accountId).pipe(take(1)).subscribe(campaignsDisplay => {
        ...
        this.getListCampaignYoutube(uid, accountId).pipe(take(1)).subscribe(campaignsYoutube => {
          let campaigns: Display[] = [...campaignsDisplay, ...campaignsYoutube]
          if (campaigns.length > 0) {
            resolve(campaigns)
          }else{
            resolve([])
    ...
    return new Promise(resolve=>{
      this.afs.collection<Search>('adwords-search', (ref)=>ref.where('id_campagne', '==', campaignId)).valueChanges().pipe(take(1)).subscribe(campaignSearch=>{
        if(campaignSearch.length>0){
          ////console.log(campaignSearch[0].createdAt)
          ...
          this.afs.collection<Search>('youtube-ads', (ref)=>ref.where('id_campagne', '==', campaignId)).valueChanges().pipe(take(1)).subscribe(campaignNative=>{
  ...
  addAdGroup(campaign_id: number, accountId: string, uid: string, name: string, strategie: string, strategie_bid: any, bid: any, targetAllPlacement: boolean): Promise<any> {
    return new Promise(resolve => {
      this.http.post(SERVER.url + '/addAdGroupSearch', {
        'ad_group_name': name,
        'campaign_id': campaign_id,
        'strategie': strategie,
  ...
  getAdGroup(id: string) {
    return this.afs.doc<AdGroupSearch>(`adgroup-search/${id}`);
  }
  ...
  updateCampaignEndDate(id: string, campaign_id: number, numberOfDays: number, endDateEnglish: string, endDateFrench: string, endDateFormattedGoogle: string, endDate: any): Promise<string> {
    return new Promise(resolve => {
      ...
      this.http.post(SERVER.url + '/updateCampaignEndDate', {
        ...
        message: 'Modification de la date de fin en cours'
      })
        .subscribe(
          res => {
            ////////////////console.log(res)
  ...
  getAdGroupAge(campaign_id: number, ad_group_id: number): Promise<AGES_TYPE[]> {
    return new Promise(resolve => {
      this.afs.collection<AdGroupSearch>('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).valueChanges().subscribe(el => {
        // resolve(el[0].ages)
      })
  ...
  getAdGroupDevices(campaign_id: number, ad_group_id: number): Promise<any> {
    return new Promise(resolve => {
      this.afs.collection('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).valueChanges().subscribe(el => {
  ...
  getAdGroupPlacementValue(campaign_id: number, ad_group_id: number): Promise<any> {
    return new Promise(resolve => {
      this.afs.collection<AdGroupSearch>('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).valueChanges().forEach(el => {

        // this.currentAdGroupTargetedPlacement = el[0].targetedPlacements
  ...
  currentAdGroupTargetedKeywords: KEYWORDS_TARGET[] = []
  getAdGroupKeywordsValue(campaign_id: number, ad_group_id: number): Promise<any> {
    return new Promise(resolve => {
      this.afs.collection<AdGroupSearch>('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).valueChanges().forEach(el => {
      
        this.currentAdGroupTargetedKeywords = el[0]?el[0].keywords:[]
        resolve('ok')
      })
  ...
  getAdGroupPlacement(campaign_id: number, ad_group_id: number): Promise<any> {
    return new Promise(resolve => {
      this.afs.collection('adgroup-search', (ref) => ref.where('campaign_id', '==', campaign_id).where('ad_group_id', '==', ad_group_id)).valueChanges().forEach(el => {
  ...
  targetKeywords(keywords: any, ad_group_id?: number, ad_group_id_firebase?: string, previous?: KEYWORDS_TARGET[]): Promise<string> {
    return new Promise(resolve => {
      if (keywords.length > 0) {
        this.http.post(SERVER.url + '/addKeywords', {
          'adGroupId': ad_group_id?ad_group_id:this.currentAdGroupId,
              'keywords': keywords,
              message: 'Paramétrage des mots clés'
        })
          .subscribe(
            (res: KEYWORDS_TARGET[]) => {
              if (res.length > 0) {
                this.updateAdgroup(ad_group_id_firebase?ad_group_id_firebase:this.currentAdGroupIdFirebase, { keywords: previous?[...previous, ...res]:res }).then(res => {
                  if (res === "ok") {
                    resolve('ok')
                  } else {
                    resolve('error')
  ...
  removeKeywords(keywords: any, ad_group_id?: number, ad_group_id_firebase?: string, previous?: KEYWORDS_TARGET[]): Promise<string> {
    return new Promise(resolve => {
      if (keywords.length > 0) {
        this.http.post(SERVER.url + '/removeKeywords', {
          'adGroupId': ad_group_id?ad_group_id:this.currentAdGroupId,
              'keywords': keywords,
              message: 'Suppression des mots clés'
        })
          .subscribe(
            (res: {status: string}) => {
              if (res.status==='ok') {
                this.updateAdgroup(ad_group_id_firebase?ad_group_id_firebase:this.currentAdGroupIdFirebase, { keywords: previous}).then(res => {
                  if (res == "ok") {
                    resolve('ok')
                  } else {
                    resolve('error')
  ...
  addCallExtension(call: PHONE_TYPE, campaignId?: number, cid?: string): Promise<string> {
    return new Promise(resolve => {
      if (call!==null) {
        this.http.post(SERVER.url + '/addCallExtension', {
          'campaignId': campaignId?campaignId:this.currentCampaignId,
          'callCountryCode': call.phoneInfo.countryCode,
          'callPhoneNumber': call.internationalNumber,
          'internationalNumber': call.internationalNumber,
          'country': call.country,
          'phoneInfo': call.phoneInfo,
          message: "Paramétrage des extensions d'appel"
        })
          .subscribe(
            (res: PHONE_TYPE[]) => {
              if (res[0].status==='ENABLED' || res[0].status==='PAUSED') {
                this.updateCampaign(cid?cid:this.currentCampaignIdFirebase, { callExtension: res[0] }).then(res => {
                  if (res === "ok") {
                    resolve('ok')
                  } else {
                    resolve('error')
  ...
  removeCallExtension(call: PHONE_TYPE, campaignId?: number, cid?: string): Promise<string> {
    return new Promise(resolve => {
      if (call.internationalNumber!==undefined) {
        this.http.post(SERVER.url + '/removeCallExtension', {
          'campaignId': campaignId?campaignId:this.currentCampaignId,
          'feedItemId': call.feedItemId,
          message: "Paramétrage des extensions d'appel"
        })
          .subscribe(
            (res: {status: string}) => {
              if (res.status==='ok') {
                this.updateCampaign(cid?cid:this.currentCampaignIdFirebase, { callExtension: null }).then(res => {
                  if (res === "ok") {
                    resolve('ok')
                  } else {
                    resolve('error')
  ...
  targetNewKeywords(keywords:[], idc: string, campaignId: number, idA: string, ad_group_id: number): Promise<string> {
    return new Promise(resolve => {
      if (keywords.length > 0) {
        this.getAdGroupKeywordsValue(campaignId, ad_group_id).then(getting => {
          if (getting === "ok") {
            this.http.post(SERVER.url + '/addKeywords', {
              ...
              .subscribe(
                (res: KEYWORDS_TARGET[]) => {
                  ...
                  if (res.length > 0) {
                    ////////////console.log('success')
                    ////////////console.log(res)
                    this.updateCampaign(idc, { keywords: [...this.currentAdGroupTargetedKeywords, ...res] }).then(response_campaign => {
                      if (response_campaign === "ok") {
                        this.updateAdgroup(idA, { keywords: [...this.currentAdGroupTargetedKeywords, ...res] }).then(response_adgroup => {
                          if (response_adgroup === "ok") {
  ...
  getCampaignDataValue__IdNotIncluded(id: string): Promise<Search> {
    return new Promise(resolve => {
  ...
  getSingleCampaignData(id: string): Promise<Search> {
    return new Promise(resolve => {
  ...
  getCampaignZones(campaign_id: number): Promise<LOCATION[]> {
    return new Promise(resolve => {
      ...
      setTimeout(() => {
        ...
        this.afs.collection<Search>('adwords-display', (ref) => ref.where('id_campagne', '==', campaign_id)).valueChanges().subscribe(el => {
          if(el[0]!==undefined){
            resolve(el[0]['targetedLocations'])

          }else{
            this.afs.collection<Search>('youtube-ads', (ref) => ref.where('id_campagne', '==', campaign_id)).valueChanges().subscribe(el => {
              if(el[0]!==undefined){
                resolve(el[0]['targetedLocations'])
    
              }else{
                this.afs.collection<Search>('adwords-search', (ref) => ref.where('id_campagne', '==', campaign_id)).valueChanges().subscribe(el => {
                  if(el[0]!==undefined){
                    resolve(el[0]['targetedLocations'])
  ...
  getCampaignZonesExcluded(campaign_id: number): Promise<LOCATION[]> {
    return new Promise(resolve => {
      ////////////////console.log(this.currentCampaignId)
      setTimeout(() => {
        ...
        this.afs.collection<Search>('adwords-display', (ref) => ref.where('id_campagne', '==', campaign_id)).valueChanges().subscribe(el => {
          if(el[0]!==undefined){
            resolve(el[0]['excludedLocations'])
  ...
  createAd(ad_id: any, accountId: string, ad_group_id: any, ad_name: any, status: any, url_image: any, image_content: any, displayUrl: any, finalUrls: any, finalMobileUrls: any, finalAppUrls: any, referenceId: any, automated: any, uid: any, size: any, ad_type: any): Promise<any> {
    return new Promise(resolve => {
      ...
      const docRef = this.afs.collection('ads-search').add(this.adsModel);
      resolve('ok')
    })
  }

  
  prepareSaveAdResponsiveSearch(ad_id: any, ad_group_id: any, ad_name: any, status: any, finalUrls: any, uid: any, titles: any, descriptions: any): ResponsiveSearchAd {
    const userDoc = this.afs.doc(`users/${uid}`);
  ...
  createAdResponsiveSearch(ad_id: any, ad_group_id: any, ad_name: any, status: any, finalUrls: any, uid: any, titles: any, descriptions: any): Promise<any> {
    return new Promise(resolve => {
      this.adsModel = this.prepareSaveAdResponsiveSearch(ad_id, ad_group_id, ad_name, status, finalUrls, uid, titles, descriptions);
      const docRef = this.afs.collection('ads-search').add(this.adsModel);
      resolve('ok')
    })
  }
        ...
        .subscribe(
          (res: AdsResponse[]) => {
            if (res.length > 0) {
              ...
              ////////console.log(res)
              res.forEach((ad, index, arr) => {
                batch.set(this.afs.collection('ads-search').doc(ad.ad_id.toString()).ref, { owner: uid, accountId: accountId, createdAt: firebase.firestore.FieldValue.serverTimestamp(), createdBy: userDoc.ref, ...ad })
  ...
  public getListDisplayAdsFormACampaign(accountId: string, ad_group_id: number) {
    ...
    return this.afs.collection<ResponsiveSearchAd>('ads-search', (ref) => ref.where('accountId', '==', accountId).where('ad_group_id', '==', ad_group_id)).snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
          const data = a.payload.doc.data();
          return { id: a.payload.doc.id, ...data };
  ...
  addAdResponsiveSearchAd(uid: string, aacid: string,  data: SEARCH_ADS_BEFORE_UPLOAD[], ad_group_id?: number): Promise<{status: string, partialErrors: adDispalyError[]}> {
    return new Promise(resolve => {
      this.http.post(SERVER.url + '/addResponsiveSearchAd', {
        'ad_group_id': ad_group_id?ad_group_id:this.currentAdGroupId,
        'data': data,
        message: 'Création des annonces en cours'

      })
        .subscribe(
          (res: {response: ResponsiveSearchAd[], errors: adDispalyError[]}) => {
            console.log(res)
            const userDoc = this.afs.doc(`users/${uid}`);
            const batch = this.afs.firestore.batch()
            if(res.response.length>0){
              res.response.forEach(async (response,idx)=>{
                batch.set(this.afs.collection('ads-search').doc(response.ad_id.toString()).ref, {owner: uid, accountId: aacid, createdAt: firebase.firestore.FieldValue.serverTimestamp(),
                  createdBy: userDoc.ref, ...response })
  ...
  getKeywordsEstimationFromWebPage(url: string[], lang: Language, lng: number): Promise<{keyword: string, volume: number}[]> {
    return new Promise(resolve => {
      lang.languageCode = lang.languageCode+'_'+lang.languageCode.toUpperCase()
      this.http.post<{keyword: string, volume: number}[]>(SERVER.url + '/getKeywordsByUrlList', {
        'urls': url,
        'lang': lang,
        'lng': lng
      })
  ...
  getKeywordsEstimationFromService(services: {CriterionID: number, Category: string}[], lang: Language, lng: number): Promise<{keyword: string, volume: number}[]> {
    return new Promise(resolve => {
      this.http.post<{keyword: string, volume: number}[]>(SERVER.url + '/getKeywordsByServicesAndProducts', {
        'data': services,
        'lang': lang,
        'lng': lng
      })
  ...
  getKeywordsEstimationFromkeywords(keywords: string[], lang: Language, lng: number): Promise<{keyword: string, volume: number}[]> {
    return new Promise(resolve => {
      lang.languageCode = lang.languageCode+'_'+lang.languageCode.toUpperCase()
      this.http.post<{keyword: string, volume: number}[]>(SERVER.url + '/getKeywordsIdea', {
        'keywords': keywords,
        'lang': lang,
        'lng': lng
      })
  ...
  removeAd(ads: AdToDelete[]): Promise<string> {
  ...
  getAd(id: string) {
    return this.afs.doc<ResponsiveSearchAd>(`ads-search/${id}`);
  }
  ...
  getSingleAd(ad_group_id: number, ad_id: number): Promise<any> {
    ...
    return new Promise(resolve => {
      this.afs.collection<Ads>('ads-search', (ref) => ref.where('ad_group_id', '==', ad_group_id).where('ad_id', '==', ad_id)).snapshotChanges().pipe(
        map((actions) => {
          return actions.map((a) => {
            const data = a.payload.doc.data();
            return { id: a.payload.doc.id, ...data };
