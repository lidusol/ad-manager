...
@Component({
  selector: 'adf-age-performance-report',
  templateUrl: './age-performance-report.component.html',
  styleUrls: ['./age-performance-report.component.scss']
})
export class AgePerformanceReportComponent implements OnInit {
    ...
    return new Promise(resolve => {
      try{
        ...
        annotations: {
          ...
          this.displayService.checkAgeRangeReport(this.aacid).then(report=>{
            if(report===null){
            ...
            this.displayService.getAgeRangePerformanceReport(this.uid, this.aacid, startDateFormattedGoogle, endDateFormattedGoogle).then(res => {
              if(res.length>1){
                this.groupAgeRangePerformanceReportByCriteria(res).then(groupByCriteria=>{
                  ...
                  let _result_ = []
                    ...
                    result.forEach((value, __index)=>{
                      ////console.log(value)
                      /* value.splice(1,1) */
                      if(value[0].toString()==='65 or more'){
                        _result_.push(['65+', value[1], value[2], value[3]])
              ...
              let _result_ = []
                ...
                result.forEach((value, __index)=>{
                  ...
                  /* value.shift() */
                  if(value[2].toString()==='65 or more'){
                    _result_.push(["65+",value[0], value[1], value[3]])
        ...
        if(index===geoTargetData.length-1){
          ...
          this.ageRangeReport.forEach((item, y)=>{
            ...
            if(this.ageRangeReport.length-1===y){
              ...
              this.displayService.saveAgeRangeReport(this.aacid, this.clicks, this.costs, this.impressions, this.ageRangeReport, this.googleVisualizeData).then(rep=>{
                /* ////////console.log(rep) */
                ////console.log(rep)
