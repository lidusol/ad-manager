...
@Component({
  selector: 'adf-campaigns-data-table-youtube',
  templateUrl: './campaigns-data-table-youtube.component.html',
  styleUrls: ['./campaigns-data-table-youtube.component.scss']
})
export class CampaignsDataTableYoutubeComponent implements OnInit {

  @ViewChild('gridCampaignsYoutube', { static: false }) gridCampaigns: GridComponent
  ...
  constructor(private youtubeService: YoutubeService, private auth: AuthService, private router: Router, private seo: SeoService, private storageService: LocalStorageService, private accountsService: AccountsService) {
  ...
  duplicateCampaign(campaign_to_duplicate: Display) {
    this.isLoading = true
    this.youtubeService.getCampaignDataValue__IdNotIncluded(campaign_to_duplicate.id).then(campaign => {
      let budget: number = 0
      ...
      new_campaign.imagesNative = new_campaign.imagesNative?new_campaign.imagesNative:[]
      ...
      this.youtubeService.saveDuplicateCampaign(new_campaign).then(campaignDuplicated=>{
        if (campaignDuplicated !== null) {
           this.youtubeService.getListCampaign(this.uid_action, this.aacid).pipe(take(1)).subscribe(campaigns => {
            this.data = campaigns
            this.dataDisplayFiltered = this.data
            this.isLoading = false
             this.router.navigate(['/campaigns/review/native'], {queryParams: {cid: campaignDuplicated.id, auid: this.uid, aacid: this.aacid}})
          })
        } else {
          this.isLoading = false
        }
     ...
     this.storageService.getUserIdAndAccountId().then(response => {
      if (response !== null) {
        ...
        if (response.fromOwned) {
          ...
          this.youtubeService.getListCampaign(response.auid, response.aacid).pipe(take(1)).subscribe(campaigns => {
            this.data = campaigns
            this.dataDisplayFiltered = this.data
            this.isLoading = false
          ...
          this.uid = response.auid
           this.youtubeService.getListCampaign(response.account.owner, response.aacid).pipe(take(1)).subscribe(campaigns => {
             this.data = campaigns
             this.dataDisplayFiltered = this.data
             this.isLoading = false
             this.getAccountValue()
