...
@Component({
  selector: 'adf-campaigns-data-table',
  templateUrl: './campaigns-data-table.component.html',
  styleUrls: ['./campaigns-data-table.component.scss']
})
  
export class CampaignsDataTableComponent {
  ...
  constructor(private displayService: DisplayService, private auth: AuthService, private router: Router, private seo: SeoService, private storageService: LocalStorageService, private accountsService: AccountsService) {
  ...
  MENU_FILTER_OPTION: { id: string, name: string }[] = [{ id: 'all', name: 'cmp_data_table.all_cmp_label' }, { id: 'enabled', name: 'cmp_data_table.all_enabled_cmp_label' }, { id: 'paused', name: 'cmp_data_table.all_paused_cmp_label'},{ id: 'ended', name: 'cmp_data_table.all_ended_campaigns' }]
  ...
  campaign_to_duplicate: Display = null
  triggerDuplication(campaign_to_duplicate: Display) {
  ...
  abortDuplication() {
   ...
   duplicateCampaign(campaign_to_duplicate: Display) {
    this.isLoading = true
     this.displayService.getCampaignDataValue__IdNotIncluded(campaign_to_duplicate.id).then(campaign => {
       let budget: number = 0
      ...
      this.displayService.saveDuplicateCampaign(new_campaign).then(campaignDuplicated=>{
        if (campaignDuplicated !== null) {
           this.displayService.getListCampaign(this.uid_action, this.aacid).pipe(take(1)).subscribe(campaigns => {
             this.data = campaigns
             this.dataDisplayFiltered = this.data
            this.isLoading = false
             this.router.navigate(['/campaigns/review/display'], {queryParams: {cid: campaignDuplicated.id, auid: this.uid, aacid: this.aacid}})
          })
        } else {
          this.isLoading = false
        }
    ...
    this.storageService.getUserIdAndAccountId().then(response => {
      if (response !== null) {
        ...
        if (response.fromOwned) {
          ...
          this.campaigns = this.displayService.getListCampaign(response.auid, response.aacid)
          this.displayService.getListCampaign(response.auid, response.aacid).pipe(take(1)).subscribe(campaigns => {
            this.data = campaigns
            this.dataDisplayFiltered = this.data
          ...
          this.uid = response.auid
          this.campaigns = this.displayService.getListCampaign(response.auid, response.aacid)
           this.displayService.getListCampaign(response.account.owner, response.aacid).pipe(take(1)).subscribe(campaigns => {
             this.data = campaigns
             this.dataDisplayFiltered = this.data
             this.isLoading = false
             this.getAccountValue()
