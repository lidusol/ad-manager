...
import { Display, PlacementReport } from 'src/app/campaigns-management/models/youtube.models';
...
@Component({
  selector: 'adf-channels-overview-recap',
  templateUrl: './channels-overview-recap.component.html',
  styleUrls: ['./channels-overview-recap.component.scss']
})
export class ChannelsOverviewRecapComponent implements OnInit {
 ...
 public toolbarOptions: ToolbarItems[];
  ...
  constructor(private route: ActivatedRoute, private accountsService: AccountsService, private youtubeService: YoutubeService, private router: Router, private storageService: LocalStorageService) { }
  campaigns: Display[] = []
  accounts: Account[] = []
 ...
 public initialPage: PageSettingsModel = { pageSize: 5 };
  ...
  getCampaignsIdList(uid: string, account_id: string): Promise<string>{
    return new Promise(resolve => {
      this.campaignIDS = []
      this.youtubeService.getListCampaign(uid, account_id).subscribe(result => {
        this.data = result
      ...
      if (response !== null) {
        ...
        if (response.fromOwned) {
          ...
          if (this.campaign_id === 0) {
            ...
            this.youtubeService.getChannelsSummuryReport(this.campaign_id).then(report => {
                  //console.log(report)
                  this.reports = report
                  this.isLoading = false
                  setTimeout(() => {
                  
                      if(this.gridCampaign!==undefined && this.gridCampaign!==null){
       ...
       /*  this.getCampaignsIdList(response.account.owner, this.adf_account_id).then(response_status => {
          if (response_status === 'ok') {
               //console.log(this.campaignIDS.join(','))
              this.youtubeService.getPlacementSummuryReport(this.campaign_id).then(report => {
                //console.log(report)
                this.reports = report
                this.isLoading = false
                setTimeout(() => {
