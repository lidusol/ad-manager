...
import { LOCATION, SCHEDULE_INTERFACE, DISPLAY_ADS, PLACEMENT_TYPE, AGES_TYPE, GENDERS_TYPE, User_Role, PHONE_TYPE, SEARCH_ADS_BEFORE_UPLOAD } from 'src/app/utils/data';
...
@Component({
  selector: 'adf-display-search-builder',
  templateUrl: './display-search-builder.component.html',
  styleUrls: ['./display-search-builder.component.scss'],
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: {showError: true}
  }]
})
export class DisplaySearchBuilderComponent implements OnInit {
...
/* CAMPAIGN DATA */
 ...
 /* END CAMPAIGN DATA  */
 constructor(private router: Router, private route: ActivatedRoute, private _formBuilder: FormBuilder, private changeDetectorRef: ChangeDetectorRef, media: MediaMatcher, private auth: AuthService, private displayService: DisplayService, private searchService: SearchService, private youtubeService: YoutubeService, private storageService: LocalStorageService) { 
 ...
 accessToCampaign() {
   this.storageService.setCid(this.new_created[0].id).then(setting_id => {
     if (setting_id === "ok") {
       //this.router.navigate(['/campaigns/review/search'], {queryParams: {cid: this.new_created[0].id, auid: this.uid, aacid: this.aacid}})
       window.location.replace(SERVER.url_redirect+`/campaigns/review/search?cid=${this.new_created[0].id}&aacid=${this.aacid}&auid=${this.uid}`)
     }
   })
...
if (this.snav.opened) {
  ...
  onDataUrlEmitted(url: string){
    this.searchAdsCreator.url_promote_subject.next(url)
  }
         ...
         this.route.queryParams.subscribe(params => {
          const adf_account_id: string = params['aacid'];
           ...
           if (response.aacid === adf_account_id && uid === response.auid && adf_account_id!==undefined && uid!==undefined) {
             if (response.role.admin) {
               ...
               this.user_access = response.role
                 ...
                 setTimeout(() => {
                   ...
                   this.KEYWORDS_SUBJECT = this.appKeywords.KEYWORDS_SUBJECT
                   this.KEYWORDS_SUBJECT.subscribe(val=>{
                     this.searchAdsCreator.KEYWORDS = val
                   })
                 }, 500);
             ...
             this.router.navigate(['/campaigns/new/search/create'], { queryParams: { aacid: response.aacid, auid: response.auid, obj: params['obj'] } })
           }
               ...
               this.getStatus().then(getting_status => {
                 ...
                 if (getting_status === "ok") {
                   this.getZones().then(getting_zones => {
                     ...
                     if (getting_zones === "ok") {
                           this.getKeywords().then(getting_demo_target => {
                             ...
                             if (getting_demo_target === "ok") {
                               this.getPack().then(getting_budget => {
                                 ...
                                 if (getting_budget === "ok") {
                                   this.getCallExt().then(getting_placement => {
