...
import {Account, RESPONSIVE_DISPLAY_ADS, User_Role} from '../../../utils/data'
...
import { NativeAdsPreviewCollectionsComponent } from '../native-ads-preview-collections/native-ads-preview-collections.component';
...
@Component({
  selector: 'adf-native-ads-overview-recap',
  templateUrl: './native-ads-overview-recap.component.html',
  styleUrls: ['./native-ads-overview-recap.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class NativeAdsOverviewRecapComponent implements OnInit {
  ...
  @ViewChild('gridCampaignsRecap', {static: false}) gridCampaign: GridComponent
   @ViewChild('adsCollections', {static: false}) adsCollections: NativeAdsPreviewCollectionsComponent
  uid: string = ""
  ...
  getData() {
    //console.log('getting for native ads')
    this.ads = []
    this.storageService.getUserIdAndAccountId().then(response => {
      if (response !== null) {
        ...
        if (response.fromOwned) {
            this.youtubeService.getListCampaign(response.auid, this.adf_account_id).subscribe(campaignsYoutube => {
              let campaigns: Display[] = campaignsYoutube
              if(campaigns.length>0){
                campaigns.forEach((campaign, index, arr) => {
                  if (campaign.id_campagne !== 0) {
                    this.youtubeService.getListAdsWithAdGroupId(campaign.ad_group_id).subscribe(ads => {
                      if(ads.length>0){
                        //console.log(ads)
               ...
               this.adsCollections.preloadImages()
                        ...
                        //console.log(this.adsCollections.isLoading)
            ...
            this.youtubeService.getListCampaign(response.account.owner, this.adf_account_id).subscribe(campaignsYoutube => {
            let campaigns: Display[] = campaignsYoutube
            if(campaigns.length>0){
              campaigns.forEach((campaign, index, arr) => {
                if (campaign.id_campagne !== 0) {
