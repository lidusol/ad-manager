...
@Component({
  selector: 'adf-cmp-search-recap',
  templateUrl: './cmp-search-recap.component.html',
  styleUrls: ['./cmp-search-recap.component.scss']
})
export class CmpSearchRecapComponent implements OnInit {

  @Input() data: Search[] = [];
  isLoading = true
  @Input() shadowed: boolean = true
constructor(private searchService: SearchService,  public translate: TranslateService, private storageService: LocalStorageService) { 
...
getKeywords(campaign: Search): number{
    if(campaign.id_campagne!==0){
        ...
        this.searchService.getAdGroupKeywordsValue(campaign.id_campagne, campaign.ad_group_id).then((res)=>{
            if(res==='ok'){
                return this.searchService.currentAdGroupTargetedKeywords.length
            }else{
                return 0
            }
        }).catch((e)=>{
...
getLocationTargeted(campaign: Search): LOCATION[]{
    if(campaign.id_campagne!==0){
        return campaign.targetedLocations
    }else{
        this.searchService.getCampaignZones(campaign.id_campagne).then((res)=>{
            if(res.length>0){
                return res
            }else{
                return []
...
getLocationExcluded(campaign: Search): LOCATION[]{
    if(campaign.id_campagne!==0){
        return campaign.excludedLocations
    }else{
        this.searchService.getCampaignZonesExcluded(campaign.id_campagne).then((res)=>{
            if(res.length>0){
                return res
            }else{
                return []
