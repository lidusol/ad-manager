...
import { LOCATION, SCHEDULE_INTERFACE, DISPLAY_ADS, PLACEMENT_TYPE, AGES_TYPE, GENDERS_TYPE, User_Role, YOUTUBE_CHANNELS_INTERFACE, YOUTUBE_VIDEOS_INTERFACE, NATIVE_ADS_TO_PUBLISH, WEBSITE } from 'src/app/utils/data';
...
import { CmpAdsUploaderYoutubeComponent } from '../single/cmp-ads-uploader-youtube/cmp-ads-uploader-youtube.component';
...
@Component({
  selector: 'adf-youtube-builder',
  templateUrl: './youtube-builder.component.html',
  styleUrls: ['./youtube-builder.component.scss'],
   providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: {showError: true}
  }]
})
export class YoutubeBuilderComponent implements OnInit {
  ...
  @ViewChild(CmpAdsUploaderYoutubeComponent, { static: false }) adsComponent: CmpAdsUploaderYoutubeComponent
...
/* CAMPAIGN DATA */
  ...
  /* END CAMPAIGN DATA  */
  constructor(private router: Router, private route: ActivatedRoute, private _formBuilder: FormBuilder, private changeDetectorRef: ChangeDetectorRef, media: MediaMatcher, private auth: AuthService,  private youtubeService: YoutubeService, private storageService: LocalStorageService) { 
  ...
  urlEmitted(url: string) {
    this.nativeAdsCreator.urlPromote = Observable.create(observer => { 
      observer.next(url)
    })
  }
  ...
  adChannel: this.objectiveComponent.current_ad_channel
      ...
      this.youtubeService.createCampaign(campaign).
        then(response => {
          if (response !== "error") {
            let new_created: Display = {id: response, ...campaign}
            this.new_created.push(new_created)
            this.opened = false
  ...
  accessToCampaign() {
    this.storageService.setCid(this.new_created[0].id).then(setting_id => {
      if (setting_id === "ok") {
        window.location.replace(SERVER.url_redirect+`/campaigns/review/youtube?cid=${this.new_created[0].id}&aacid=${this.aacid}&auid=${this.uid}`)
        //this.router.navigate(['/campaigns/review/youtube'], {queryParams: {cid: this.new_created[0].id, auid: this.uid, aacid: this.aacid}})
      ...
      this.storageService.getUserIdAndAccountId().then(response => {
              ...
              if (response.role.admin) {
                ...
                this.router.navigate(['/campaigns/list'])
						  ...
						  this.router.navigate(['/campaigns/new/native/create'], { queryParams: { aacid: response.aacid, auid: response.auid, obj: params['obj'] } })
					  }
                              ...
                              if (getting_demo_target === "ok") {
                                this.getPack().then(getting_budget => {
                                  if (getting_budget === "ok") {
                                    this.getChannels().then(getting_channels => {
                                      if (getting_channels === "ok") {
                                        this.getSchedules().then(getting_schedules => {
                                          if (getting_schedules === "ok") {
                                            this.getAssetsNative().then(getting_native => {
                                              if (getting_native === 'ok') {
                                                this.saveData()
                                              } else {
      ...
      this.adsComponent.uploadDisplayImages().then(res => {
        if (res === "error") {
         if (this.adsComponent.selectedImagesFromGallery.length > 0) {
            ...
            this.spinnerCreation = false
            this.openSnackbar(8, 'Aucune annonce valide !', 'ok', 'snack-danger')
            resolve('error')
